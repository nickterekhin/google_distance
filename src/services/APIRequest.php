    ...
    private function run($endpoint,$params=array(),$method=self::HTTP_METHOD_GET,array $headers=null)
    {
        switch($method)
        {
            ...
            case self::HTTP_METHOD_PATCH:
                $this->request_options[CURLOPT_POSTFIELDS] = json_encode($params);
                $this->request_options[CURLOPT_CUSTOMREQUEST] = self::HTTP_METHOD_PATCH;
                break;
        }
    ...
    private function request($url)
    {
        try {
            ...
            if (200 != $http_status && 201 != $http_status) {
                throw new CustomExceptions($this->request_result, $http_status);
            }
            curl_close($ch);
        }catch(CustomExceptions $e) {
            throw new CustomExceptions(sprintf('Error: %s', $e->getMessage()));
        } catch(Exception $e) {
